...
This module contains integration tests for the RAG pipeline functionality,
including document loading, embedding generation, FAISS indexing, and
...
def rag_basic_test():
    """Test basic RAG pipeline functionality with default model."""
    logger.info("=== Testing RAG Pipeline with default model ===")

    try:
        rag = RAGPipeline()
        logger.info(
            f"RAG Pipeline initialized with model: {rag.embedding_model.model_name}"
        )
